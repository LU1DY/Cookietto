...
function verifyInputForm() {
  ...
  if (inputEmailValue === "") {
    messageError("Informe seu e-mail!", errorElement[0]);
    applyErrorStyles(inputEmail.parentElement);
  } else if (!emailPattern.test(inputEmailValue)) {
    messageError("Informe um e-mail válido!", errorElement[0]);
    applyErrorStyles(inputEmail.parentElement);
  } else {
  ...
  if (inputPasswordValue === "") {
    messageError("Informe sua senha!", errorElement[1]);
    applyErrorStyles(inputPassword.parentElement);
  } else if (inputPasswordValue.length < 8 || inputPasswordValue.length > 20) {
    messageError("A senha deve ter de 8 a 20 caracteres!", errorElement[1]);
    applyErrorStyles(inputPassword.parentElement);
  } else {
    removeErrorStyles(inputPassword.parentElement);
  }

  clearErrorOnInput(inputEmail, errorElement[0]);
  clearErrorOnInput(inputPassword, errorElement[1]);
}

btnSubmitLogin.addEventListener("click", verifyInputForm);
